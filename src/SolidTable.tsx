...
export function Tree({
  ...
  return <ol className={classnames('list-tree', className)} role="tree" style={{
    position: 'relative',
    ...style
  }} {...props} />;
}
...
export class TreeItem extends React.Component<TreeItemProps> {
  ...
  render() {
    ...
    return <div title={title}>
        <li aria-selected={selected} className={classnames(className, {
        selected
      }, 'list-item')} onMouseDown={onMouseDown} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} data-path={path} data-name={name} onClick={this._handleClick} ref={liNode => this._liNode = liNode} role="treeitem" tabIndex={selected ? '0' : '-1'}>
...
export class NestedTreeItem extends React.Component<NestedTreeItemProps> {
  ...
  render() {
    ...
    return <li aria-selected={selected} aria-expanded={!collapsed} className={classnames(className, {
      selected,
      collapsed
    }, 'list-nested-item')} onClick={this._handleClick} role="treeitem" tabIndex={selected ? '0' : '-1'}>
        {title == null ? null : <div tabIndex={-1} className="native-key-bindings list-item" ref={node => this._itemNode = node}>
            {title}
          </div>}
